...
void KeyBoard::timerHandleSlot()
{
    this->cur_time += 1;
    // 计算打字速度
    unsigned long speed_1 = this->cur_charactors * 60 / this->cur_time;
    if(!this->is_warning){
        if(speed_1 >= 200){

            // qDebug()<<"加时前的速度："<<speed_1<<" "<<"加时前的时间"<<cur_time;
            cur_time += 5;
            unsigned long speed_2 = this->cur_charactors * 60 / this->cur_time;
...
void KeyBoard::on_view1_textChanged()                   // 采用无connect的信号与槽连接 其中格式为 on_对象名_信号
{
    ...
    int len = str.length();
    int num_new = len - cur_handled_pos - 1; //  输入的新字符个数
    int num_last = cur_show_str.length() - cur_handled_pos - 1;  //  剩下的字符个数
    ...
    /*  处理 cur_handled_pos 开始到结束的字符   
        cur_handled_pos 此时处在上一次输入字符后的最有一个字符的左侧 = len-1或者-1
        判断输入的字符与目标字符是否相同
    */ 
    for (int i = 0; i < num; ++i) {
...
void KeyBoard::backspace()
{
    ...
    // qDebug()
    /*找到光标操作删除最后一个字符*/
    ui->view1->backspace(); //  QLineEdit->backspace() 删除 QLineEdit 中当前光标的字符
    ui->view1->setFocus();  //  聚焦
    --cur_handled_pos;
...
// 当 KeyBoard 窗口关闭时,会显示其父窗口 TextList。
